...
export async function getCustomers(search?: string): Promise<DbCustomer[]> {
  ...
  let query = supabase
    .from("customers")
    .select(
      "id, name, phone, email, birthday, notes, is_vip, allergies, tags, loyalty_points, stamp_count, afm, address, contact, billing, is_active, discount, legacy_id, source, metadata, last_visit_at, total_visits, total_spent, marketing_opt_in, tier_id, tier_updated_at, lifetime_points, points_expiring_at, created_at, updated_at",
    )
    .eq("is_active", true)
